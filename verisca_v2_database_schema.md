# Verisca V2 Database Schema - Complete Design
...
CREATE TABLE growth_stages (
    ...
    typical_days_from_planting INTEGER, -- Typical timing
...
CREATE TABLE perils (
    ...
    severity_levels JSONB, -- Definition of severity categories
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE lookup_table_definitions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    table_code VARCHAR(50) UNIQUE NOT NULL, -- 'STAND_REDUCTION_EARLY', 'HAIL_LEAF_LOSS'
    table_name VARCHAR(100) NOT NULL,
    table_description TEXT,
    crop_id UUID REFERENCES crops(id), -- NULL for crop-agnostic tables
    method_id UUID REFERENCES assessment_methods(id),
    source_reference VARCHAR(100), -- 'USDA_EXHIBIT_11', 'FAO_TABLE_3'
    ...
    table_type VARCHAR(30) NOT NULL, -- 'matrix', 'key_value', 'multi_dimensional'
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Assessment Sessions with Results
CREATE VIEW assessment_sessions_with_results AS
SELECT 
    s.id,
    s.claim_id,
    c.claim_number,
...
-- Insert Core Perils
INSERT INTO perils (peril_code, peril_name, peril_category, description) VALUES
('DROUGHT', 'Drought/Insufficient Moisture', 'weather', 'Insufficient soil moisture for normal plant development'),
('HAIL', 'Hail Damage', 'weather', 'Direct and indirect damage from hailstorms'),
('HEAT_STRESS', 'Heat Stress/Hot Winds', 'weather', 'Excessive heat causing plant stress and yield reduction'),
('COLD_DAMAGE', 'Cold Weather/Frost', 'weather', 'Low temperatures causing plant damage or death'),
('DISEASE_FUNGAL', 'Fungal Disease', 'biological', 'Damage from fungal pathogens'),
('DISEASE_BACTERIAL', 'Bacterial Disease', 'biological', 'Damage from bacterial pathogens'),
('DISEASE_VIRAL', 'Viral Disease', 'biological', 'Damage from viral pathogens'),
('PEST_INSECT', 'Insect Pest Damage', 'biological', 'Damage from insect pests'),
('PEST_WILDLIFE', 'Wildlife Damage', 'biological', 'Damage from birds, mammals, and other wildlife'),
('FLOODING', 'Flooding/Excessive Moisture', 'weather', 'Damage from standing water and excessive soil moisture'),
('EMERGENCE_FAILURE', 'Emergence Failure', 'environmental', 'Poor or failed crop emergence');
...
('USDA_2025.1', 'USDA Corn Loss Adjustment Standards 2025', 'USDA_FCIC25080', '2024-11-18', '2025-01-01',
 'Complete digitization of USDA FCIC-25080 Corn Loss Adjustment Standards Handbook',
 '["MAIZE"]', '["STAND_REDUCTION", "HAIL_DAMAGE", "MATURITY_LINE_WEIGHT", "WEIGHT_METHOD", "TONNAGE_METHOD"]',
 true, 'approved');
```

This completes the comprehensive database schema. The design is:

1. **Future-proof**: Easily extensible for new crops
2. **Multi-tenant**: Complete data isolation per insurer
3. **Offline-ready**: Full sync architecture
4. **Audit-compliant**: Complete change tracking
5. **Scalable**: Optimized indexes and relationships
6. **Generic**: Lookup tables work for any crop
7. **Comprehensive**: Supports the entire assessment workflow

The schema supports everything from claim intake through final report generation while maintaining flexibility for future enhancements.
